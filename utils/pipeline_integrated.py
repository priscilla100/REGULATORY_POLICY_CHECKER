"""
Integrated Pipeline Implementation with JSON + FOTL Two-Tier Verification
...
3. INTEGRATED TWO-TIER VERIFICATION:
   - Tier 1A: Pattern matching (fast)
   - Tier 1B: LLM + procedural JSON (flexible)
   - Tier 2: OCaml + primary FOTL (rigorous)
...
# UPDATED: Use integrated verifier instead of old wrapper
...
def validate_and_fix_formula(formula: str) -> Tuple[str, List[str], List[str]]:
    ...
    if formula.startswith('forall'):
        ...
        if match:
            ...
            # Exclude keywords and predicates
            keywords = {'forall', 'implies', 'and', 'or', 'not', 'true', 'false'}
            predicates = set(ARITY_MAP.keys())
            used_vars = used_vars - keywords - predicates
...
Policy: {', '.join(verification_result.policy_citations) if verification_result.policy_citations else 'N/A'}
...
Be clear and reassuring."""
        ...
        # Tier 1B: LLM procedural match
        explain_prompt = f"""Explain this HIPAA compliance result to a non-technical user.

Question: {query}
Result: COMPLIANT (LLM Procedural Classification - Tier 1B)
Exception: {verification_result.procedural_exception.name if verification_result.procedural_exception else 'N/A'}
Reason: {verification_result.explanation}

Provide:
1. Clear YES answer
2. Brief explanation citing the procedural policy
3. Policy citation
...
# ============================================================================
# MAIN PIPELINE WITH INTEGRATED TWO-TIER VERIFICATION
# ============================================================================

def pipeline_with_two_tier_verification(query: str, client: Anthropic) -> Dict:
    """
    Complete pipeline with INTEGRATED two-tier verification (JSON + FOTL)
    
    Flow:
    1. LLM1: Extract facts
    2. LLM1: Translate to formula
    3. Integrated Two-Tier Verification (Pattern → LLM+JSON → OCaml+FOTL)
    4. LLM2: Generate explanation
    ...
    try:
        ...
        # ===================================================================
        # STEP 3: INTEGRATED TWO-TIER VERIFICATION (UPDATED!)
        # ===================================================================
        
        steps.append("")
        steps.append("⚙️ STEP 3: Integrated Two-Tier Verification (JSON + FOTL)...")
        ...
        integrated_verifier = create_integrated_verifier(client, ocaml_verifier)
        
        # Run verification
        verification_result = integrated_verifier.verify(query, facts, formula)
        
        # Log results based on tier
        if verification_result.tier == VerificationTier.TIER_1A_PATTERN:
            steps.append(f"✅ Tier 1A: Pattern Match")
            ...
            steps.append("   → No further verification needed!")
        
        elif verification_result.tier == VerificationTier.TIER_1B_PROCEDURAL:
            steps.append(f"✅ Tier 1B: LLM Procedural Classification")
            if verification_result.procedural_exception:
                steps.append(f"   Exception: {verification_result.procedural_exception.name}")
                steps.append(f"   Source: {verification_result.procedural_exception.source}")
            steps.append(f"   Confidence: {verification_result.confidence:.2%}")
        ...
        return {
            "name": "Integrated Two-Tier Pipeline ⭐",
            "answer": explanation,
            ...
            "method": "Integrated Two-Tier: JSON Routing + Pattern + LLM + FOTL",
